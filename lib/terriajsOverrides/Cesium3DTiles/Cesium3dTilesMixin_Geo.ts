...
import AbstractConstructor from "terriajs/lib/Core/AbstractConstructor";
...
import ClippingMixin from "terriajs/lib/ModelMixins/ClippingMixin";
...
interface Cesium3DTilesCatalogItemIface
  extends InstanceType<ReturnType<typeof Cesium3dTilesMixin_Geo>> {}
...
function Cesium3dTilesMixin_Geo<T extends AbstractConstructor<BaseType>>(Base: T) {
  abstract class Cesium3dTilesMixin extends ClippingMixin(
     ...
     * 
     * MODIFIED: Calculates the earth radius at given coordinates and subtracts that from
     * the original Cartesian3 object to allow for custom height offsets.
     */
    private computeModelMatrixFromTransformationTraits(modelMatrix: Matrix4) {
      ...
      if (latitude !== undefined && longitude !== undefined) {
        ...
        // Check to make sure the height value is defined, else warn user.
        // This ideally should never happen.
        let h = 1;
        if (height !== undefined) {
          h = height;
        } else {
          console.warn("Height value is undefined for geocentric 3d tile.");
        }

        let earthRadius = Cartesian3.fromDegrees(
          longitude,
          latitude,
          0
        );

        let offset = Cartesian3.fromDegrees(
          longitude,
          latitude,
          h
        )

        position = Cartesian3.subtract(offset, earthRadius, position);

        /*
        // Original code from Cesium3dTilesMixin.ts
...
namespace Cesium3dTilesMixin_Geo {
...
export default Cesium3dTilesMixin_Geo;
